        ...
        <div className="flex gap-2 overflow-x-auto scroll-hidden items-center pb-1 -mx-4 px-4">
          ...
          <div className="flex gap-2 items-center flex-shrink-0">
            {(Object.entries(DAY_LABELS) as [DayCode, string][]).map(([code, label]) => (
              <button
                ...
                className={`filter-pill whitespace-nowrap flex-shrink-0 active:scale-95 touch-manipulation ${selectedDay === code ? 'filter-pill-active' : ''}`}
              >
                {label.split(', ')[0]}
              </button>
            ))}
          ...
          <div className="flex gap-2 items-center flex-shrink-0">
            {TIME_SLOTS.map((slot) => (
              <button
                ...
                className={`filter-pill whitespace-nowrap flex-shrink-0 active:scale-95 touch-manipulation ${selectedTimeSlot === slot ? 'filter-pill-active' : ''}`}
              >
                {slot}
              </button>
            ))}
          ...
          {hasFilters && (
            <>
              ...
              <button 
                onClick={onClearAll} 
                className="filter-pill text-primary hover:text-primary-foreground hover:bg-primary flex items-center gap-1 whitespace-nowrap flex-shrink-0 active:scale-95 touch-manipulation"
              >
